...
// Authorization nếu dùng middleware verifyToken lấy token từ header
router.get("/authentication_header", verifyToken, (req, res, next) => {
  res.send("OK");
});

// nếu sử dụng passport.authenticate("jwt", { session: false }) thì phải có JwtStrategy
router.get(
  "/authentication",
...
router.post("/", validateSchema(customerBodySchema), async (req, res, next) => {
  try {
    ...
    if (checkEmailExist) {
      return res.status(400).json({ message: "exist account" });
    }
